...
//  static double Cos270 = Math::cos(Math::PI * 3.0 / 2.0);// 270 degrees
  /* ********************************************************************************* */
  void Cubic_Spline_Chunk(Point2D& p0, Point2D& p1, Point2D& p2, Point2D& p3, double FractAlong, Point2D& result) {
  ...
  /* ********************************************************************************* */
  void Cubic_Spline_Boxes(ArrayList<IOffsetBox>& raw, int NumSubLines, Point2D* SplinePoints) {// Splines that only go forward in time
    double CtrlPntLength = 1.0 / 2.0;
    Point2D Prev, Next(0, 0);
